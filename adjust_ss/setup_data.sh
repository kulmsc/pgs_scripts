...
if [ ! -e geno_files/${lowauthor}.${chr}.bed ];then
  ...
  plink2_new --bgen temp_files/${author}.${chr}.bgen ref-first --sample ~/athena/ukbiobank/imputed/ukbb.${chr}.sample --keep-fam ~/athena/doc_score/qc/cv_files/train_eid.${train_frac}.txt --maf 0.01 --geno 0.1 --hwe 1e-50 midp --make-bed --out geno_files/${lowauthor}.${chr}

...
cat all_specs/prscs_param_specs | while read phi;do
  ...
  if [ ! -e ~/athena/doc_score/mod_sets/${author}/${low_author}.prs.${i}.ss ]; then
    ...
    python ~/Programs/PRScs/PRScs.py --ref_dir=/home/kulmsc/athena/refs/ldblk_1kg_eur --bim_prefix=geno_files/${low_author}.${chr} --sst_file=${d}/ss --phi=0.000001 --n_gwas=${ess} --chrom=$chr --out_dir=${d}/output.$i

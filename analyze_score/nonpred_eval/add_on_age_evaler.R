...
df <- split_age(df)
print(paste(author, max(df$age[df$young_old == 0])))
exit()
...
so_far_done <- readRDS(paste0("per_score_results/", tolower(author), ".res.RDS"))
so_far_done[["age_diff"]] <- all_age_auc
saveRDS(so_far_done, paste0("per_score_results/", tolower(author), ".res.wage.RDS"))
